...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0) {
			throw new TreeException("A tree has said you're wrong"); 
		}
		...
		{
			parentNode = root.getNodeWithName(parent);  						//HERE IS THE ISSUE There's a method in Node that 																									//searches for a named node. 
																			//??? If the parent node wasn't found, there must have been something 																				//wrong in the  data file. Throw an exception.
			if (parentNode == null) {
				throw new TreeException("IT AINT THERE");
			}
		}
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("EXCEPTION"); 	//Throw a TreeException with a useful message
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			throw new TreeException("EXCEPTION"); 	//Throw TreeException with a useful message
